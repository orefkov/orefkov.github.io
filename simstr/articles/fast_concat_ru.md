...
```
Но если для скрипт-детишек простительно не думать о том, что стоит за такой простой записью, для опытного
...
[результаты всех замеров с Compiler Explorer](https://godbolt.org/z/c5YjfoP88).
```
...
строкового выражения, т.е. он сам является строковым выражением, и к нему можно по цепочке снова
...
Мутабельные типы строк в simstr также умеют использовать их при вставке и заменах, аналогично
получая необходимую длину, готовя место и размемщая символы выражения в это место.
...
со строковым литералом. Таким образом, `"The answer is " + str_answer` - создает новый временный объект - строковое выражения. Прибавление к нему `", count is "` - создает следующий строковый объект, хранящий ссылку
...
Бенчмарки различных вариантов ей использования можно посмотреть [здесь](https://orefkov.github.io/simstr/results.html).
